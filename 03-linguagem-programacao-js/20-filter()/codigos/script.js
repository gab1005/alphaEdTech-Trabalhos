...
//arrayClientes = [a,b,c,d]; a-nome, b-data de compra, c-data de vencimento, d-valor produto
//arrayMontante = []; array com todos os montantes;
...
console.log("dataInicial.getDate()", dataInicial.getDate());
console.log("dataFinal.getDate()", dataFinal.getDate());

function filtrarData() {
    let dataInicial = new Date(document.querySelector(".filtro-data-inicio").value);
    let dataFinal = new Date(document.querySelector(".filtro-data-final").value);

    function aplicarFuncao(item) {
        console.log("------------------");
        if (diferencaDatas(item.c, dataInicial) > 0) {
    ...
    let arrrayResultado = arrayClientes.filter(aplicarFuncao);
