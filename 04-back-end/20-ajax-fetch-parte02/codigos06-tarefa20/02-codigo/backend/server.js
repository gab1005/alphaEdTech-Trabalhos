...
app.get("/pesquisar", (req, res) => {
    ...
    let arrResponse = clientes.filter(elem => {
        let idData = parseInt(elem.id);
        let nameData = elem.name.toLowerCase();
        let emailData = elem.email.toLowerCase();

        if (idData == id || nameData.includes(name) || emailData.includes(email)) {
            return true;
        }
